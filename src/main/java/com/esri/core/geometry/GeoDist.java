 ...
 */
 ...
 final class GeoDist {
	 ...
	 static private void setPeDouble(PeDouble p, double val){
		 if(p != null) p.val = val;
	 }
 
	 // Helper function to compute geodesic distance in case of antipodal points
	 static private boolean geodesic_distance_antipodal(double a, double e2, double lam1,
		 ...
		 if (PE_EQ(PE_ABS(phi1), PE_PI2)) {
			 ...
			 setPeDouble(p_dist, 2.0 * q90(a, e2));
			 if(phi1 > 0.0){
				 setPeDouble(p_az12, lam_delta(PE_PI - lam_delta(lam2)));
				 setPeDouble(p_az21, lam_delta(lam2));
			 }else{
				 setPeDouble(p_az12, lam_delta(lam2));
				 setPeDouble(p_az21, lam_delta(PE_PI - lam_delta(lam2)));
			 }
			 return true;
			 ...
			 setPeDouble(p_dist, 2.0 * q90(a, e2));
			 setPeDouble(p_az12, 0.0);
			 setPeDouble(p_az21, 0.0);
			 return true;
		 }
		 return false;
	 }
	 ...
	 static public void geodesic_distance_ngs(double a, double e2, double lam1,
		 ...
		 if (PE_EQ(phi1, phi2) && (PE_ZERO(dlam) || PE_EQ(PE_ABS(phi1), PE_PI2))) {
			 ...
			 if(geodesic_distance_antipodal(a,e2,lam1,phi1,lam2,phi2,p_dist,p_az12,p_az21, dlam))
				 return;
		 }
		 ...
		 if (p_az12 != null || p_az21 != null) {
			 ...
			 setPeDouble(p_az12, lam_delta(az12));
			 setPeDouble(p_az21, lam_delta(az21));
		 }
	 }
 }
